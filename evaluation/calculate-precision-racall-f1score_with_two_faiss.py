...
# グローバル変数の初期化
# paper_number = 500
# index = faiss.read_index("./predict/faiss_index_with_all_faster_nbits_per_idx4.index")
...
def calculate_precision_recall(args):
    attention_paper_id, attention_reference_works , paper_number, index = args
...
# 並列計算関数
def parallel_process(paper_number, index):
    conn = sqlite3.connect('./predict/paper2.db')
    cursor = conn.cursor()
    ...
    while flag < 50:
        ...
        if attention_reference_works:
            tasks.append((attention_paper_id, attention_reference_works, paper_number, index))
            flag += 1
        count += 1

    conn.close()

    with Pool(processes=8) as pool:  # 16プロセスで並列化
...
if __name__ == "__main__":
    ...
    paper_numbers = [500, 5000]
    times = []
    mean_precisions_list = []
    mean_recalls_list = []
    mean_f1_scores_list = []
    index2 = faiss.read_index("./predict/faiss_index_with_all_faster_nbits_per_idx4.index")
    print("FAISS index loaded successfully.")
    ...
    for paper_number in paper_numbers:
        for i in range(2):
            ...
            all_f1_scores = {}
            if (i == 0):
                results = parallel_process(paper_number, index)
                ...
                results = parallel_process(paper_number, index2)
   ...
   # Plotting the graphs for Precision, Recall, and F1-Score
    ...
    for i in range(4):
        if (i == 0):
            axes[0].plot(topN_list, mean_precisions_list[i], marker='o', label=f'Paper Number {paper_numbers[0]}')
        if (i == 1):
            axes[0].plot(topN_list, mean_precisions_list[i], marker='o', label=f'Paper Number {paper_numbers[0]} quantize')
        if (i == 2):
            axes[0].plot(topN_list, mean_precisions_list[i], marker='o', label=f'Paper Number {paper_numbers[1]}')
        if (i == 3):
            axes[0].plot(topN_list, mean_precisions_list[i], marker='o', label=f'Paper Number {paper_numbers[1]} quantize')
    ...
    for i in range(4):
        if (i == 0):
            axes[1].plot(topN_list, mean_recalls_list[i], marker='o', label=f'Paper Number {paper_numbers[0]}')
        if (i == 1):
            axes[1].plot(topN_list, mean_recalls_list[i], marker='o', label=f'Paper Number {paper_numbers[0]} quantize')
        if (i == 2):
            axes[1].plot(topN_list, mean_recalls_list[i], marker='o', label=f'Paper Number {paper_numbers[1]}')
        if (i == 3):
            axes[1].plot(topN_list, mean_recalls_list[i], marker='o', label=f'Paper Number {paper_numbers[1]} quantize')
    ...
    for i in range(4):
        if (i == 0):
            axes[2].plot(topN_list, mean_f1_scores_list[i], marker='o', label=f'Paper Number {paper_numbers[0]}')
        if (i == 1):
            axes[2].plot(topN_list, mean_f1_scores_list[i], marker='o', label=f'Paper Number {paper_numbers[0]} quantize')
        if (i == 2):
            axes[2].plot(topN_list, mean_f1_scores_list[i], marker='o', label=f'Paper Number {paper_numbers[1]}')
        if (i == 3):
            axes[2].plot(topN_list, mean_f1_scores_list[i], marker='o', label=f'Paper Number {paper_numbers[1]} quantize')
    ...
    output_filename = 'precision_recall_f1_scores_with_two_faiss.png'
